...
int add_file_to_dumplist(uint32_t id, uint8_t type, uint8_t attrib,
                         char *filename) {
  FILE *dumpfile;
  char name[512];
  char tmp[512];
...
void print_help() {
  ...
  fprintf(stdout, "Arguments: \n"
                  ...
                  "\t-o: Output directory\n"
                  "\t-d: Print hex dumps\n"
                  "\t-f: Fix carrier ID (make it's ID be below 0x06000000)");
}
...
int process_nv_configuration_data() {
  ...
  for (int i = 0; i < num_items; i++) {
    ...
    case MCFG_ITEM_TYPE_UNKNOWN:
      ...
      fprintf(stdout, " - [NV] Item %i (#%i) @offset %ld: %s \n", i, nvitem->id,
      ...
      for (int k = 0; k < 2; k++) {
        ...
        case EFS_FILECONTENTS:
          ...
          if (efsfilenametmp[0] != 0x00) {
            ...
            add_file_to_dumplist(item->id, item->type, item->attrib,
                                 get_efsitem_as_filename(efsfilenametmp, i));
          } else {
            fprintf(stderr, "EFS File dump: Filename is empty!\n");
          }
          break;
        }
        fprintf(stdout, " - [File] Item %i (#%i) @offset %ld: %s \n", i,
    ...
    case MCFG_ITEM_TYPE_FOOT:
      if (debug)
        fprintf(stdout, "Footer at %ld bytes, size of %ld bytes\n",
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "i:o:hdf")) != -1)
    switch (c) {
    case 'i':
      if (optarg == NULL) {
